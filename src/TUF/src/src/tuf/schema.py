...
class ListOf(Schema):
  ...
  def check_match(self, object):
    if not isinstance(object, (list, tuple)):
      raise tuf.FormatError('Expected '+self._list_name+'; got '+repr(object))
    ...
    for item in object:
      try:
        ...
        raise tuf.FormatError(str(e)+' in '+self._list_name)
    ...
    if not (self._min_count <= len(object) <= self._max_count):
        raise tuf.FormatError('Length of '+self._list_name+' out of range')
...
class Object(Schema):
  ...
  def check_match(self, object):
    if not isinstance(object, dict):
      raise tuf.FormatError('Wanted a '+self._object_name+' did not get a dict')
    ...
    for key, schema in self._required:
      ...
      try:
        ...
        if not isinstance(schema, Optional):
          raise tuf.FormatError('Missing key '+key+' in '+self._object_name)
...
class Struct(Schema):
  ...
  def check_match(self, object):
    if not isinstance(object, (list, tuple)):
      raise tuf.FormatError('Expected '+self._struct_name+'; got '+repr(object))
...
class RegularExpression(Schema):
  ...
  def check_match(self, object):
    if not isinstance(object, basestring) or not self._re_object.match(object):
      raise tuf.FormatError(repr(object)+' did not match '+self._re_name)
