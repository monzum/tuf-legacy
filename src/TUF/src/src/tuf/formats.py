...
<Purpose>
  A central location for all format-related checking of objects.
  ...
  This module can be broken down into three sections.  (1) Schemas and object
  ...
  signable_object = make_signable(signed_object)
...
# A relative file path (e.g., '/meta/roles/').
...
# A list of TERGETFILE_SCHEMA objects.
...
class TimestampFile(MetaFile):
  def __init__(self, timestamp, expires, meta_files):
    self.info = {}
  ...
  def from_metadata(object):
    ...
    meta_files = object['meta']
    return TimestampFile(timestamp, expires, meta_files)
    
    
  @staticmethod
  def make_metadata(meta_filedict):
...
class TargetsFile(MetaFile):
  def __init__(self, timestamp, expires, targets=None, delegations=None):
    if targets is None:
      targets = {}
  ...
  def from_metadata(object):
    ...
    timestamp = parse_time(object['ts'])
    expires = parse_time(object['expires'])
    targets = object.get('targets')
    delegations = object.get('delegations')
    
    return TargetsFile(timestamp, expires, targets, delegations)


  @staticmethod
  def make_metadata(targets_filedict=None, delegations=None):
...
class MirrorsFile(MetaFile):
  ...
  @staticmethod
  def from_metadata(object):
    # Is 'object' a Mirrorlist metadata file?
    # Raise tuf.FormatError if not.
    MIRRORLIST_SCHEMA.check_match(object)

    timestamp = parse_time(object['ts'])
    expires = parse_time(object['expires'])
    mirror_list = object['mirrors']

    return MirrorsFile(timestamp, expires, mirror_list)


  @staticmethod
  def make_metadata(mirror_list):
    """
    mirrors_list is a list of MIRROR_SCHEMA objects.
    """
...
def format_base64(binary_base64):
  """
  <Purpose>
    Return the base64 encoding of 'binary_base64' with whitespace
    and '=' signs omitted.

  <Arguments>
    binary_base64:
      Binary data representing a base64 value.

  <Exceptions>
    tuf.FormatError, if the base64 encoding fails.
...
def make_signable(object):
  """
  <Purpose>
    Return the role meta 'object' in SIGNABLE_SCHEMA format.
    'object' is added to the 'signed' key, and an empty
    list initialized to the 'signatures' key.  The caller adds
    signatures to this second field.
    Note: check_signable_object_format() should be called after
    make_signable() and signatures are added to ensure the final
    'signable' object has the correct format.

  <Arguments>
    object:
      A role schema dict (e.g., ROLE_SCHEMA, RELEASE_SCHEMA). 
...
def make_fileinfo(length, hashes, custom=None):
  ...
  <Arguments>
    ...
    hashes:
      A list of hashes in HASHDICT_SCHEMA format, which has the form:
      {'length': 4242,
       'hashes': {'sha256': 123df8a9b12, 'sha512': 324324dfc121, ...}
       'custom': {...}}
...
def make_role_metadata(keyids, threshold, paths=None):
  ...
  <Purpose>
    ...
    as field values.  'paths' is optional (used only by the 'Target'
    role).
...
def check_signable_object_format(object):
  ...
  <Arguments>
    object:
     The object being tested against SIGNABLE.SCHEMA. 
...
def canonical_string_encoder(string):
  """
  <Purpose>
...
def _encode_canonical(object, output_function):
  ...
  if isinstance(object, basestring):
    output_function(canonical_string_encoder(object))
  elif object is True:
    ...
    if len(object):
      ...
      for key, value in items[:-1]:
        output_function(canonical_string_encoder(key))
        output_function(":")
    ...
    raise tuf.FormatError('I can\'t encode '+repr(object))
...
def encode_canonical(object, output_function=None):
  ...
  <Exceptions>
    tuf.FormatError, if 'object' cannot be encoded.
