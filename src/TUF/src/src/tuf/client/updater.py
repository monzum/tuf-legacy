...
<Purpose>
  ...
  systems need to utilize.  It provides a single class representing a
  repository that includes methods to download, install, and verify
...
<Example Client>
  ...
  # The repository may now be instantiated.  The Repository class of updater.py
  # is called with two arguments.  The first argument assigns a name to this
  # particular repository and the second argument the repository mirrors defined
  # above.
  ...
  updated_targets = repository.updated_targets(targets, destination_directory)
...
class Repository(object):
  """
  <Purpose>
    Provide a class that can download target files securely.  The repository
  ...
  <Repository Attributes>
    self.metadata:
      ...
      Ex: {'current': {'root': ROOTROLE_SCHEMA,
  ...
  def __init__(self, repository_name, repository_mirrors):
    """
    <Purpose>
      Constructor.  Instantiating a repository object causes all the metadata
    ...
    <Exceptions>
      ...
      tuf.RepositoryError:
        If there is an error with the repository's
        state, such as a missing 'root.txt' file.
    ...
    if tuf.conf.repository_directory is None:
      message = 'The TUF update client module must specify the directory' \
                ' containing the local repository\'s files.' \
    ...
    if not os.path.exists(current_path):
      message = 'Missing '+repr(current_path)+'.  This path must exist and, ' \
                'at a minimum, contain the "root.txt" metadata file.' 
      raise tuf.RepositoryError(message)
  ...
  def __str__(self):
    """
      The string representation of a repository object.
  ...
  def _update_metadata(self, metadata_role, compression=None):
    ...
    # The metadata has been verified. Move the metadata files into place.
    ...
    # Now move the verified updated metadata file to the 'current' directory.
    # Note that the 'move' method comes from util's TempFile class.
    # 'metadata_file_object' is an instance of this class.
    ...
    current_metadata_object = self.metadata['current'][metadata_role]
  ...
  def _ensure_all_targets_allowed(self, metadata_role, metadata_object):
    """
    <Purpose>
      Ensure the delegated targets of 'metadata_role' is allowed; this is
  ...
  def _targets_of_role(self, rolename, targets=None, skip_refresh=False):
    ...
    <Returns>
      A dict object containing the target information of all the targets
      of 'rolename'.  Conformant to 'tuf.formats.TARGETFILES_SCHEMA'.
